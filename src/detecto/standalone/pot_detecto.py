...
def compute_extreme_anomaly_threshold(
    dataset: DataFrame, total_anomaly_score_feature: str, t1: int, q: float = 0.80
) -> float:
    """
    Calculate the anomaly threshold with quantile method for detecting the anomalies.

    # Parameters
    ------------
    * dataset (DataFrame): A Pandas DataFrame that holds all the anomaly scores from the genpareto fitting method.
    * total_anomaly_score_feature (str): The name of feature that holds the total anomaly score per row.
    * t1 (int): Time window used to define the amount of timeframe to learn and calculate the quantile score.
    * q (float): The quantile, ranging from 0.0 - 1.0 with default value of 0.80.

    # Returns
    ------------
        * float: The threshold for detecting the anomalous data.
    """
    clean_dataset = dataset[
        (dataset[total_anomaly_score_feature] > 0) & (dataset[total_anomaly_score_feature] != float("inf"))
    ]
    ...
    return quantile(
        a=clean_dataset.iloc[:t1][total_anomaly_score_feature].to_list(),
        q=q,
    )


def detect_extreme_anomaly(
    dataset: DataFrame, total_anomaly_score_feature: str, t1: int, extreme_anomaly_threshold: float
) -> DataFrame:
    """
    ...
    * dataset (DataFrame): A Pandas DataFrame that holds all the anomaly scores from the genpareto fitting method.
    * total_anomaly_score_feature (str): The name of feature that holds the total anomaly score per row.
    * t1 (int): Time window used to get the time of interest.
    * extreme_anomaly_threshold (float): The anomaly threshold that will be compared with all values in t2 time window.

    # Returns
    ------------
        * DataFrame: The result is a Pandas DataFrame with boolean values where `True` indicates an anomaly.
    """
    anomaly_data = {}
    t2_dataset = dataset.iloc[t1:]
    anomaly_data["is_anomaly"] = (
        t2_dataset[total_anomaly_score_feature].apply(lambda x: x > extreme_anomaly_threshold).to_list()
    )
